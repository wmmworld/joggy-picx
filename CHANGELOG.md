...
### Added
- [Claude + CEO] **Bib detector v1 — production-ready** (`apps/backend/models/yolov8n_bib.onnx`, 12 MB; gitignored, deployed manually): YOLOv8-nano trained on Roboflow project `wmm-qv1ad/joggy-bib` version 1 (9,397 images: train 9,276 / val 76 / test 45, 640×640, 3× augmentation, single class `bib` merged from 6 source datasets). Trained on Kaggle (Tesla T4 ×2, 100 epochs, AdamW auto-tune, patience=20). **Final metrics:** mAP50 = **0.914**, mAP50-95 = 0.675, precision ~0.91, recall ~0.85. Exported to ONNX (`opset=17, simplify=True`) on Kaggle (avoided installing ultralytics+torch on Windows dev box where C: was nearly full). Tensor signature: input `images` [1,3,640,640], output `output0` [1,5,8400] — matches `joggy.ai.bib_detector.BibDetector` (D-021 compliant)
- [Claude] **`BibDetector.detect_all()` + NMS** (`apps/backend/joggy/ai/bib_detector.py`): new method returns `list[BibBox]` (all bibs in one photo) after greedy NMS at IoU > 0.45. Legacy `detect()` retained as backward-compat wrapper returning `detect_all()[0]` or `None` so `joggy.worker.pipeline.py` (which still assumes 1 bib/photo for the OCR + DB schema) doesn't break. `_CONF_THRESHOLD` dropped 0.5 → **0.25** (YOLOv8 default) after CEO eyeball test on holdout showed occluded/oblique bibs were being filtered out — recall > precision matters more for the Joggy use case because the downstream OCR step naturally filters false positives (a box on a hand can't be read as digits and falls through to the manual review queue). 4 new tests added (10/10 pass, was 6): empty list, multiple distinct bibs, NMS drops overlap, legacy `.detect()` returns top-conf box. Multi-bib downstream pipeline (PhotoBib table + worker loop + Public API join + review UI) tracked as backlog item — needs ADR before implementing
- [Claude] **`tools/train/test_holdout.py`** — production-stack sanity check script. Loads `apps/backend/models/yolov8n_bib.onnx` through the real `BibDetector` class (onnxruntime + opencv + numpy only — no ultralytics/torch import, so this script proves the production codepath works end-to-end). Verifies ONNX input/output tensor names against `BibDetector` expectations. Iterates `joggy_dataset/holdout/`, calls `detect_all()`, draws green bbox + `#N conf` label for every bib found, writes annotated copies to `joggy_dataset/holdout_detected/` for visual review. Holdout result on first run: **20/20 images had ≥1 bib (100%) · 78 total bibs detected (avg 3.9/image, max 11)**. CEO eyeball check flagged 2 known limitations for v2: bibs farther than ~20px in original image are missed (Z50_0239/0240 background runners) + bibs occluded by hands/phones are missed (AOF_R6_-1319). 2 false-positive boxes (conf < 0.3 on a hand) accepted as cheap noise since the OCR step filters them
- [Claude] **`tools/export/export_yolo.py` updated** — path now `models/best.pt` (Kaggle-trained weights) → `apps/backend/models/yolov8n_bib.onnx`. Ultralytics writes `best.onnx` next to `best.pt`; script now mkdir-parents and shutil.move into place. Dev script only — never imported by production worker (per D-021)
- [Claude] **`apps/backend/models/README.md` expanded** — documents `yolov8n_bib.onnx` v1: training provenance (Roboflow project + version + class merge story), validation metrics (mAP50 0.914 / mAP50-95 0.675), holdout eyeball-test result (20/20 images, 78 bibs), known v2 limitations, regenerate-from-scratch checklist, and the `tools/train/test_holdout.py` sanity-check command. So the next person who needs to refresh the model doesn't have to spelunk PROGRESS.md to figure out where it came from

### Fixed
- [Claude] **`joggy.ai.session.load_sessions()` now fails fast with a clear message** (`apps/backend/joggy/ai/session.py`): before this commit, a missing ONNX file made onnxruntime raise `[ONNXRuntimeError] : 3 : NO_SUCHFILE` without saying which path it tried — devs had to grep the source to figure out what was expected. Now the loader pre-checks every required path with `os.path.isfile` and, if anything is missing, raises `FileNotFoundError` listing every missing file and pointing at `apps/backend/models/README.md`. Also refactored the hard-coded `_load(...)` calls into a declarative `_MODEL_PATHS` dict so the path list is a single source of truth (used by both the existence check and the session construction). 2 new tests added (5/5 pass, was 3): `test_load_sessions_raises_clear_error_when_files_missing` (all 5 missing → all 5 listed in error + README hint), `test_load_sessions_reports_only_missing_files` (4 present, 1 missing → only the missing one mentioned, with count). Existing 3 tests updated to patch `os.path.isfile` alongside `ort.InferenceSession` since they use fake paths. Full backend suite 98/98 pass
- [Claude] **DEV-3 watchdog self-heal for pipeline** (`apps/backend/joggy/worker/recovery.py`): closes the loop opened by DEV-1. Scans recent AuditLog rows for `context.enqueue_status="pending_enqueue"` and re-enqueues them via RQ. Skips photos that already have a `enqueue_recovered` audit marker (idempotent). Stops the sweep on first ConnectionError so it doesn't burn cycles when Redis is still down. Triggered automatically on backend startup (lifespan hook) and manually via admin endpoint `POST /internal/worker/reenqueue-pending`. 6 new tests covering empty / happy / idempotency / Redis-still-down / status-filter / legacy-audit-rows cases (92/92 backend tests pass, was 86) (commit: e12a872)
...
### Fixed
- [Claude] **DEV-1: Ingest endpoint is graceful when Redis is down** (`apps/backend/joggy/api/ingest.py`): previously `enqueue_process_photo()` failure propagated as HTTP 500 → DB ROLLBACK → R2 object orphan → Pi retry storm with new UUID each time → R2 dup pile-up. Now: catch all enqueue exceptions, log WARNING, return 202 with `job_id: null` and `status: "pending_enqueue"`, audit log records the failure mode. DB commit proceeds, photo + audit row durable. Pipeline survives Redis outage; watchdog (DEV-3, future) can re-enqueue from audit log entries. 2 regression tests added (86/86 pass, was 84) (commit: 714648d)
...
### Added
- [Claude] **`tools/stress_test.py` + stress test report** — pre-production load test. 3 scenarios via httpx ASGITransport: (A) /ingest/photos burst 20 req/s × 60s with one Event Token, (B) /v1/public/photos 100 RPS × 60s, (C) rate-limit fast-fire 200 req. CLI: `--scenario`, `--duration`, `--short`, `--report`. Mocks DB/R2/RQ but Redis is real (Docker Compose). Results: 20,555 req total, 0 × 5xx, p95 16-31ms across scenarios, rate limit fired 914 × 429 → empirically verifies Codex's H-002 ingest rate-limit fix. Limitation documented: in-process — staging VPS load test is follow-up (commit: a9fdc61)
- [Claude] Phase 5 mobile-responsive UI: 6 files updated for mobile optimization — `layout.tsx` viewport meta (width=device-width, initialScale=1); dashboard responsive padding (p-4 md:p-6) + h1 sizing (text-2xl md:text-3xl); events page full-width button mobile + header sizing; event detail flex-wrap action buttons + breadcrumb text-xs sm:text-sm + photo gallery button full-width sm:w-auto; photo gallery (PRIMARY): search input text-base (prevent iOS zoom), grid 2-col mobile (grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 md:gap-4), aspect-square image container, lightbox touch-none, pagination flex-wrap; review queue stats bar flex-wrap + table min-w-[600px]. TypeScript check 0 errors ✅ (commit: ae87c84)
...
### Fixed
- [Claude] **Pi capture must be USER systemd service, not system** (`apps/edge/infra/joggy-capture.user.service`, `apps/edge/infra/99-joggy-canon.rules`): system-level service (`User=pi` ใน `/etc/systemd/system/`) รัน gphoto2 นอก login session → libgphoto2 ตอบ `Permission denied` ทุกครั้งที่ download image เพราะ systemd-logind ตั้ง uaccess ACL บน Canon USB endpoint ให้เฉพาะ login session ของ user `pi` แม้ว่า device permission จะเป็น `crw-rw-r--+ root plugdev` และ pi อยู่ใน plugdev group ก็ตาม. Fix: รันเป็น user-level service (`~/.config/systemd/user/`) + `sudo loginctl enable-linger pi` (คง user session ตลอดแม้ reboot/no login). Belt-and-braces: udev rule กำหนด MODE 0664 GROUP plugdev บน Canon EOS RP USB device (04a9:32e2) เผื่อ uaccess behavior เปลี่ยน. เพิ่ม `ExecStartPre=gphoto2 --reset` ล้าง orphan PTP session จาก unclean exit ครั้งก่อน (กล้องตอบ PTP "Access Denied" ถ้า session เก่ายังเปิดค้าง). README docs deployment steps + troubleshooting entries. Verified end-to-end ผ่าน service: shutter press → `/home/pi/photos/inbox/20260603_140116.jpg` (5.8 MB) → `Uploaded photo_id=266f30c9-...` → dashboard (commit: f988f00) ✅
- [Claude] **Pi capture systemd unit — strftime escape** (`apps/edge/infra/joggy-capture.service`): `--filename ".../%Y%m%d_%H%M%S.jpg"` ทำให้ systemd กิน `%Y/%m/%d/%H/%M/%S` เป็น systemd specifiers (state dir UUID, hostname ฯลฯ) ก่อนส่งให้ gphoto2 → filename เพี้ยนเป็น path ที่ไม่มีอยู่จริง → capture fail ทุกครั้ง → restart loop. Fix: escape เป็น `%%Y%%m%%d_%%H%%M%%S` ให้ systemd pass literal `%` ไปให้ gphoto2 strftime. ผลข้างเคียง: pipeline พังเงียบมานานเพราะ service `active (running)` แต่ไม่ trigger capture จริง — diagnose ยากเพราะดูปกติ. เพิ่ม hardening: `ExecStartPre=/bin/sleep 5` (USB enumeration race), `Restart=always` (recover from clean exit), `TimeoutStartSec=60` (commit: 444b51e)
...
- [Claude] Pi auto-start (Phase 5 hardware milestone): `apps/edge/infra/joggy-capture.service` — systemd service for `gphoto2 --capture-tethered` that depends on `joggy-edge.service`. ExecStartPre kills `gvfs-gphoto2-volume-monitor` to prevent USB conflict (with `-` prefix so failure is ignored when gvfs not running). Restart on-failure every 10s — Pi can boot before camera is plugged in, gphoto2 retries until USB device detected. Verified: Pi reboot → both services come up → live capture → upload → Dashboard in <30s, fully unattended. Token expiry handled by `EVENT_TOKEN` rotation in `/home/pi/joggy/.env`.
